...
def main():
    ...
    with open("log.txt", "w") as log_file:
        ...
        for data_leak_name in leak_names:
            ...
            print_and_log(f"\nProcessing data leak: {data_leak_name}", log_file)
            ...
            print_and_log("\nExecuting data extraction...", log_file)
            ...
            if verbose >= 0:
                print_and_log(f"\nExtracted data: {len(df.index)} passwords", log_file)
            if verbose > 0:
                print_and_log(str(df), log_file)

            # Execute data analysis
            print_and_log("\nExecuting data analysis...", log_file)
            ...
            print_and_log(f"\nAnalysis completed successfully. Check the file '{output_file}' for results.", log_file)

        # Calculate the total elapsed time for all leaks
        total_end_time = time.time()
        total_time_elapsed = total_end_time - total_start_time
        print_and_log(f"\nTotal time elapsed for all data leaks: {total_time_elapsed:.2f} seconds.", log_file)
