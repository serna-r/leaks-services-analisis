...
def write_latex_table(data, output_path):
    with open(output_path, 'w') as file:
        ...
        file.write("\\hline\n")
        file.write("\\textbf{Source} & \\textbf{Date} & \\textbf{Number of Users} & \\textbf{Mean Length} & \\textbf{Mean Score} \\\\\n")
        file.write("\\hline\n")
        # Loop through the data dictionary and fill the table with appropriate values
        for source, stats in data.items():
            num_users, mean_length, mean_score = stats
            # The Date column is left empty as per your table structure
            file.write(f"{source} &  & {num_users} & {mean_length:.2f} & {mean_score:.2f} \\\\\n")
        file.write("\\hline\n")
    ...
    for leak, _ in leak_types:
        ...
        try:
            ...
            with open(leak_file, 'r') as f:
                ...
                leak_data[leak] = (count, mean_length, mean_score)
